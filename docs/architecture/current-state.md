...
- **Testing:** `vitest` with `happy-dom` for DOM tests (596 tests / 75 files)
...
| Responsive toolbar | ✅ Complete | element-level `ResizeObserver`; quick-filter contraction, icon-only secondary controls, then delegated Export/Columns overflow menu |
| Saved grid views | ✅ Complete | serializable public capture/apply API; exact/merge column restore plus grouping, sort, filters, quick filter, and group expansion; application-owned toolbar CRUD callbacks |
...
Tests use **vitest** with `happy-dom` for DOM environment simulation — **596 tests across 75
...
- **Contextual keyboard-shortcut reference** — the grid does not yet provide an in-grid view of the
  shortcuts currently available for the focused context. The future surface must react to active
  navigation mode, focus, editing/selection state, enabled features, and platform modifier labels.
