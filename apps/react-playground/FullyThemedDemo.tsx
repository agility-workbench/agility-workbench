...
import { ColumnType, Grid, type ReactColDef } from "@react-grid";
import { themeDark, type GridTheme } from "@grid";
...
export function FullyThemedDemo() {
  const [rowHeight, setRowHeight] = useState(58);
  const theme = useMemo(() => makeTheme(rowHeight), [rowHeight]);
  ...
  const columns = useMemo<ReactColDef[]>(() => [
    ...
    { colId: "team", key: "team", label: "CREW", width: 115, filter: true },
    {
      ...
      cellStyle: ({ value }: { value: ProjectRow["stage"] }) => ({
    ...
    },
    {
      ...
      cellStyle: ({ value }: { value: ProjectRow["priority"] }) => ({
    ...
    },
    {
      ...
      cellStyle: ({ value }: { value: number }) => ({ color: value >= 75 ? "#67e8f9" : "#c4b5fd", fontWeight: "700" }),
    },
  ...
  return (
    <section className="full-theme-demo">
      <header className="full-theme-hero">
        <div>
          <span className="full-theme-kicker">CUSTOM THEME / REACT</span>
        ...
        <label className="full-theme-density">
          Row height
          <select value={rowHeight} onChange={(event) => setRowHeight(Number(event.target.value))}>
      ...
      <div className="full-theme-grid-shell">
        <Grid
          key={rowHeight}
          rowData={ROWS}
          columnDefs={columns}
...
export default FullyThemedDemo;
