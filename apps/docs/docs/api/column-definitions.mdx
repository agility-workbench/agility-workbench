...
`exportable`, and `editable` govern user capabilities. `suppressColumnPanel`
omits a column only from the management drawer.
...
`cellStyle`, and `tooltip` defaults from `getRowPresentation`. Set it to `false`
to opt out completely, or use an object such as `{ tooltip: false }`. Column
classes compose with row defaults, column inline-style properties override
conflicts, and explicit column tooltip fields override the corresponding row
defaults.
