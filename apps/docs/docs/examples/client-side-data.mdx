...
The index is evaluated after removals in the same transaction. It defaults to
the end, and out-of-range values are clamped. It is a source-order position:
active sorting, filtering, grouping, tree relationships, or pagination may put
the inserted row at a different displayed position.
...
Row nodes are mutated immediately and transactions retain call order. The
returned promise resolves after the batch is reflected in the derived view and
renderer. Until then, direct row-node reads can see new data while sorted,
filtered, or grouped positions still describe the previous render.
...
addIndex, update, remove })` remains the most direct way to express an incremental change
