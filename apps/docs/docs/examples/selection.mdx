...
- `rowSelection: { checkboxes: true }` adds a dedicated checkbox column
  ...
  covering the select-all scope (`selectAllScope`). The column is pinned left by
  default and can be pinned right or unpinned from its header menu.
