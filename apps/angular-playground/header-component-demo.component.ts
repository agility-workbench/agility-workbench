 ...
 *
 * The header components themselves are framework-agnostic core class/function components, ported
 * from the React demo unchanged.
 */
...
class FancyHeaderCell implements IHeaderComponent {
  ...
  private paint(params: HeaderComponentParams) {
...
function styleMiniButton(btn: HTMLButtonElement) {
  btn.type = "button";
  btn.style.cursor = "pointer";
  btn.style.border = "1px solid var(--pte-border, #d1d5db)";
...
@Component({
  selector: "header-component-demo",
  standalone: true,
  imports: [AwbGrid],
  template: `
    <div class="demo-intro">
      <strong>Custom header components.</strong>
      <span style="color: #0ea5e9">🗺️ Region</span> uses a Level 1 <code>headerComponent</code>
      (content only — grid keeps the filter icon, and clicking the arrow still sorts).
      <span style="font-weight: 700">Sales Rep</span> uses a Level 2 <code>headerCellComponent</code>
      (owns the whole cell; its ↕ button sorts, ⛃ opens the filter — turns green when active).
      <span style="font-style: italic; color: #6366f1">Country / Units / Revenue</span> fall
...
export class HeaderComponentDemoComponent {
  readonly rows = buildRows(500);

  readonly defaultColDef: NgDefaultColDef = {
    headerComponent: DefaultTintedHeader,
